...
parser.add_argument('--model', '-m', help = 'Path to joint BERT NLU model', type=str,default="saved_models/joint_bert_model", required=False)
parser.add_argument('--data', '-d', help='Path to data in Goo et al format', type=str, default="data/snips/valid", required=False)
parser.add_argument('--batch', '-bs', help='Batch size',
...
def get_results(input_ids, input_mask, segment_ids, valid_positions, sequence_lengths, tags_arr,
    ...
    f1_score = metrics.f1_score(
        list(chain.from_iterable(gold_tags)), list(chain.from_iterable(predicted_tags)), average='micro')
